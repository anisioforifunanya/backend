...
import NewMessage from '@/app/home/(routes)/messages/components/NewMessage';
       ...
       {data? (
           <>
              <NewMessage 
                ...
                authUser={user}
                receiver={data.posted_by}
              />
              <SectionHeader text={data.title} canUpdate={isUpdateVisible} id={data.id} type="Product"/>
               <div className="flex w-full justify-between items-center mb-4">
                    <div className='w-full'>
                        <p className='font-semibold text-2xl mb-4 text-lime-700'>
                ...
                <div>
                  ...
                  <p className="py-2 px-3 bg-red-100 whitespace-pre-wrap rounded-xl mb-4">
                    {data.description}
                  </p>
                  <p className='font-semibold text-xl mb-1'>Contact Seller</p>
                    <div className='flex justify-center items-center flex-wrap mb-4 gap-3'>
                      ...
                      <a 
                        className='flex gap-2 rounded-lg p-3 font-semibold text-white w-[48%] flex-grow hover:bg-red-900 bg-red-700 transition-colors items-center' 
                        href={`https://wa.me/${data.phone_no?.replaceAll(' ', '')}?text=Hello,+I+am+interested+in+the+${data.title.replaceAll(' ', '+')}+you+listed+on+Amebo+Connect`} 
                        target="_blank" 
                        rel="noopener noreferrer">
                        <FaWhatsapp />
                        WhatsApp ({data.phone_no})
                        </a>
                    </div>
