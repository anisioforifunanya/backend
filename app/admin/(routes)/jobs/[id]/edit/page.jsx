     ...
     {data? <form onSubmit={handleSubmit}>
      <div className={styles.inputsWrapper}>
        ...
        <button 
          type="submit" 
          disabled={isLoading}
          className={`w-full p-3 text-white rounded hover:scale-[1.02] transition flex gap-2 items-center justify-center font-bold ${isLoading? "cursor-progress bg-gray-700" : 'cursor-pointer bg-red-900'}`}>{isLoading? 'Loading': 'Submit'} {isLoading && <Loader />}</button>
        </div>
      </form>
      : <Spinner type="" /> }
    </div>
